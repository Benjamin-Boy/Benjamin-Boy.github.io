  ...
  return (
    <div className={darkTheme ? "dark" : null}>
      ...
      >
        ...
        <div className="w-[80%] h-full flex flex-col justify-between gap-2 xl:flex-row xl:justify-center xl:flex-wrap xl:items-center xl:w-[60%]">
          {worksData.map(
            ({ id, name, description, github, host, image, type, tags }) => {
              return (
                <div
                  key={id}
                  style={{
                  ...
                  className="relative overflow-hidden flex flex-col grow xl:w-[45%] h-[35%] shadow-[#181818]"
                >
                  <div className="absolute origin-center z-20 w-full flex gap-1 p-2">
                    {tags.map(({ skillId, skillName, skillIcon }) => {
                      return (
                        ...
                        >
                          {skillIcon}
                          <h3 className="z-20 px-2 text-[#181818] bg-[#fdfaf0] text-md absolute top-4 left-4 opacity-0 group-hover:opacity-100">
                            {skillName}
                          </h3>
                        </div>
                      );
